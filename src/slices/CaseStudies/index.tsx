...
const CaseStudies = async ({
  ...
  // Fetch case study from case study ID
  ...
  return (
    ...
    >
      ...
      <div className="prose prose-invert mx-auto mt-6 max-w-md text-balance text-center text-slate-300">
        <PrismicRichText field={slice.primary.body} />
      </div>

      <div className="mt-20 grid gap-32">
        {caseStudies.map(
          (caseStudy, index) =>
            caseStudy && (
              <div
                key={caseStudy.id}
                className="relative grid gap-4 opacity-85 transition-opacity duration-300 hover:cursor-pointer hover:opacity-100 md:grid-cols-2 md:gap-20 lg:grid-cols-3"
              >
                <div className="prose prose-invert col-span-1 flex flex-col justify-center">
                ...
                <PrismicNextImage
                  ...
                  className={clsx(
                    ...
                    index % 2 && "md:-order-1", // Alternate images and text
                  )}
                />
              </div>
            ),
